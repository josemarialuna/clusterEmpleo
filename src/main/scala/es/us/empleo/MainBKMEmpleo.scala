...
import org.apache.spark.mllib.clustering.BisectingKMeansEmpleo
...
object MainBKMEmpleo {
  def main(args: Array[String]) {
    ...
    val bkm = new BisectingKMeansEmpleo().setK(numClusters).setMaxIterations(numIterations)
    val clusters = bkm.run(parsedData)

    var elapsed = (System.nanoTime() - start) / 1000000000.0
    println("TIEMPO TOTAL: " + elapsed)

    val resultadoClustering = porfin.mapValues(arr => clusters.predict(Vectors.dense(arr)))

    resultadoClustering.sortBy(_._1).saveAsTextFile("trabajoBKM" + destino)
    ...
    val rddResultadoCardinal = resultadoClustering.map(x => (x._2, 1)).reduceByKey(_ + _).sortBy(_._1).saveAsTextFile("cardinalResultadoBKM" + destino)
    val rddOriginalCardinal = originalCluster.map(x => (x._2, 1)).reduceByKey(_ + _).sortBy(_._1).saveAsTextFile("cardinalOriginalBKM" + destino)
    ...
    rddAccuracy.saveAsTextFile("comparativaBKM" + destino)
