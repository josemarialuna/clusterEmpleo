...
object MainKMeans {
  def main(args: Array[String]) {
    ...
    resultadoClustering.sortBy(_._1).saveAsTextFile("trabajo" + destino)
    ...
    val rddResultadoCardinal = resultadoClustering.map(x=>(x._2,1)).reduceByKey(_+_).sortBy(_._1).saveAsTextFile("cardinalResultado" + destino)
    val rddOriginalCardinal = originalCluster.map(x=>(x._2,1)).reduceByKey(_+_).sortBy(_._1).saveAsTextFile("cardinalOriginal" + destino)
    ...
    rddAccuracy.saveAsTextFile("comparativa" + destino)
