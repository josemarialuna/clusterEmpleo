...
class KMeansEmpleo private(
  ...
  /**
    ...
    */
  @Since("0.8.0")
  def this() = this(2, 20, 1, KMeansEmpleo.K_MEANS_PARALLEL, 5, 1e-4, Utils.random.nextLong())
      ...
      val totalContribs = data.mapPartitions { points =>
        ...
        points.foreach { point =>
          (0 until runs).foreach { i =>
            val (bestCenter, cost) = KMeansEmpleo.findClosest(thisActiveCenters(i), point)
            costAccums(i) += cost
      ...
      for ((run, i) <- activeRuns.zipWithIndex) {
        ...
        while (j < k) {
          ...
          if (count != 0) {
            ...
            //println("NUEVOCENTRO")
            if (KMeansEmpleo.fastSquaredDistance(newCenter, centers(run)(j)) > epsilon * epsilon) {
              changed = true
            }
  ...
  : Array[Array[VectorWithNorm]] = {
    ...
    while (step < initializationSteps) {
      ...
      costs = data.zip(preCosts).map { case (point, cost) =>
        Array.tabulate(runs) { r =>
          math.min(KMeansEmpleo.pointCost(bcNewCenters.value(r), point), cost(r))
        }
    ...
    val weightMap = data.flatMap { p =>
      Iterator.tabulate(runs) { r =>
        ((r, KMeansEmpleo.findClosest(bcCenters.value(r), p)._1), 1.0)
      }
...
object KMeansEmpleo {
  ...
  private[clustering] def fastSquaredDistance(
    ...
    var totalDist = 0.0
    //println("CALCULO DISTANCIA")
    for (z <- 1 to 9280) {
      var minAux = 0.0
      try {
        val line = v1.vector.apply(z)
        val linePlus = v2.vector.apply(z)
